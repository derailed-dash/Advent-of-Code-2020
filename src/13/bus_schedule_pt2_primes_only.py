...
def find_aligned_buses(buses):
    ...
    for i, bus in enumerate(buses):
        ...
        if bus != "x":
            # bus_num (schedule), offset from first bus, depart timestamp
            # set the depart timestamp to 0, and then update as we process
...
def find_aligned_bus(bus1, bus2):
    ...
    lcm = bus1[0] * bus2[0]
    timestamp = get_timestamp(bus2, bus1)

    print(f"Returning {(lcm, 0, timestamp)}")
    return lcm, 0, timestamp


def get_timestamp(bus2, bus1):
